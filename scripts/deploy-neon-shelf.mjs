...
const source = await readFile(resolve(process.cwd(), "contracts/NeonShelf.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "NeonShelf.sol": { content: source } },
  settings: {
...
const contract = output.contracts?.["NeonShelf.sol"]?.NeonShelf;
if (!contract) {
...
console.log(`Deploying NeonShelf from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_NEON_SHELF_CONTRACT_ADDRESS to this contract address in Vercel.");
